...
export const index = async (req, res) => {
  ...
  if (feed === 'contactos') {
    ...
    const contactos = await Usuario.findByPk(userId, {
      include: [
        {
          model: Usuario,
          as: 'contactos',
    ...
    const contactosIds = contactos.contactos.map((c) => c.id);
...
export const like = async (req, res) => {
  ...
  if (!usuarioId) {
    return res
      .status(400)
      .json({ message: 'Debe proporcionar un usuarioId para dar like' });
  }

  console.log('Like request - novedadId:', novedadId, 'usuarioId:', usuarioId);
  ...
  if (existingLike) {
    console.log('Like ya existe para usuarioId:', usuarioId);
  ...
  const nuevoLike = await Like.create({ novedadId, usuarioId });
  console.log('Nuevo like creado:', nuevoLike.toJSON());
  const likesCount = await Like.count({ where: { novedadId } });

  res.json({
    message: 'Novedad likeada exitosamente',
    data: { likesCount, liked: true },
  });
};
...
export const unlike = async (req, res) => {
  ...
  if (!usuarioId) {
    return res
      .status(400)
      .json({ message: 'Debe proporcionar un usuarioId para dar unlike' });
  }

  console.log(
    'Unlike request - novedadId:',
    novedadId,
    'usuarioId:',
    usuarioId
  );
  ...
  console.log('Likes eliminados:', deleted);
  if (!deleted) {
    return res
      .status(404)
  ...
  res.json({
    message: 'Novedad un-likeada exitosamente',
