...
export const index = async (req, res) => {
  ...
  if (feed === 'contactos') {
    ...
    if (!estudianteId) {
      return res.status(400).json({
        message: 'Debe proporcionar un estudianteId para ver el feed de contactos',
      });
    }
  ...
  const novedadesConLikes = novedades.rows.map((n) => {
    ...
    if (data.sesion?.creador?.Usuario) data.sesion.creador = data.sesion.creador.Usuario;
    const likesCount = data.likes ? data.likes.length : 0;
    const liked = estudianteId
      ? data.likes?.some((l) => Number(l.usuarioId) === Number(estudianteId)) || false
      : false;
    return {
      ...data,
      likesCount,
...
export const like = async (req, res) => {
  ...
  if (!estudianteId || isNaN(estudianteId)) {
    return res
      .status(400)
      .json({ message: 'Debe proporcionar un estudianteId válido para dar like' });
  }
  ...
  const existingLike = await Like.findOne({ where: { novedadId, usuarioId: estudianteId } });
  if (existingLike) {
...
export const unlike = async (req, res) => {
  ...
  const deleted = await Like.destroy({ where: { novedadId, usuarioId: estudianteId } });
  if (!deleted) {
    return res
      .status(404)
