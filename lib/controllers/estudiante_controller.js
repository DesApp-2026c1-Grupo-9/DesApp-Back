...
const obtenerTodosLosEstados = async (estudianteId) => {
  const registros = await EstudianteMateria.findAll({
    where: { estudianteId: parseInt(estudianteId) },
    attributes: ['materiaId', 'estado'],
  });
  const map = new Map();
...
export const importarMateriasDesdeExcel = async (req, res) => {
  try {
    ...
    const nombreAId = new Map(
      todasLasMaterias.map((m) => [m.nombre.toLowerCase().trim(), m.id])
    );

    const resultados = { importadas: [], ignoradas: [], errores: [] };

    for (const fila of materias) {
      const nombreNorm = (fila.nombre || '').toLowerCase().trim();
    ...
    res.json({
      message: `Importación completada. ${resultados.importadas.length} materias importadas.`,
      data: resultados,
    });
  } catch (error) {
...
export const getAsistenteAcademico = async (req, res) => {
  try {
    ...
    materiasConEstado.forEach((m) => {
      ...
      else if (m.estado === 'cursando') analisisPorAnio[anio].cursando++;
      else analisisPorAnio[anio].faltantes++;
    });
