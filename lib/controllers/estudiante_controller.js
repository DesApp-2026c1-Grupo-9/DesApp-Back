...
export const getPlanMaterias = async (req, res) => {
  try {
    ...
    aniosOrdenados.forEach((anio) => {
      materiasPorAnioTemp[anio].forEach((materia) => {
        ...
        if (estadoGuardado) {
          ...
          // Asignar estado basado en año Y correlatividades
          let estado;

          if (!prerequisitosCumplidos(materia.id)) {
            // Si no cumple prerrequisitos, no puede estar aprobada/regularizada
            estado = 'no_cursada';
          } else {
            // Si cumple prerrequisitos, asignar según año
            if (materia.anio === 1) {
          ...
          estadosTemp.set(materia.id, estado);
        }
      });
    });
    ...
    const materiasConEstado = materias.map((materia) => {
      const estado = estadosTemp.get(materia.id);
  ...
  try {
    ...
    if (estado === 'aprobada' || estado === 'regularizada') {
      ...
      for (const correlatividad of correlatividades) {
        ...
        const estadoPrereq =
          obtenerEstadoMateria(estudianteId, prereqId) || 'no_cursada';

        if (estadoPrereq === 'no_cursada') {
          prerequisitosIncumplidos.push({
      ...
      try {
        ...
        if (materiasQueDependen.length > 0 && !confirmarCascada) {
          ...
          return res.status(409).json({
            ...
            message:
              'Esta materia es correlativa de otras materias. Al marcarla como "no cursada", también deberían cambiarse sus dependientes.',
            materiasAfectadas: materiasAfectadas.map((materia) => ({
              id: materia.id,
              nombre: materia.nombre,
              anio: materia.anio,
              tipo: materia.tipo,
        ...
        if (confirmarCascada && materiasQueDependen.length > 0) {
          console.log(
            `Aplicando cascada a ${materiasQueDependen.length} materias dependientes...`
          );
        }
      } catch (error) {
