...
const normalizarNombreMateriaEquivalente = (nombre) => {
...
const obtenerClaveEquivalenciaMateria = ({ codigo, nombre }) => {
  const codigoNormalizado = normalizarTexto(codigo);
  if (codigoNormalizado) {
    return `codigo:${codigoNormalizado}`;
  }

  const nombreNormalizado = normalizarNombreMateriaEquivalente(nombre);
...
const ANIOS_VIGENCIA_REGULARIDAD = 2;
...
const guardarEstadoMateria = async (estudianteId, materiaId, estado) => {
  ...
  const claveEquivalencia = obtenerClaveEquivalenciaMateria(materia || {});

  let materiasEquivalentes = [];
  if (normalizarTexto(materia?.codigo)) {
    materiasEquivalentes = await Materia.findAll({
      where: { codigo: materia.codigo },
      attributes: ['id', 'nombre', 'codigo'],
    });
  } else if (claveEquivalencia) {
    const materiasCandidatas = await Materia.findAll({
      attributes: ['id', 'nombre', 'codigo'],
    });

    materiasEquivalentes = materiasCandidatas.filter(
      (candidata) =>
        obtenerClaveEquivalenciaMateria(candidata) === claveEquivalencia
    );
  }

  if (materiasEquivalentes.length === 0 && materia?.id) {
    materiasEquivalentes = [materia];
  }

  console.log(
    `Materias equivalentes para "${materia?.nombre}": ${materiasEquivalentes.length}`
  );
...
const obtenerRegistrosEstados = async (estudianteId) => {
  return EstudianteMateria.findAll({
    ...
    include: [{ model: Materia, attributes: ['codigo', 'nombre'] }],
    order: [['updatedAt', 'DESC']],
  });
};

const obtenerFechaEstado = (registro) =>
  registro?.updatedAt || registro?.createdAt || null;

const calcularFechaVencimientoRegularidad = (registro) =>
  sumarAnios(obtenerFechaEstado(registro), ANIOS_VIGENCIA_REGULARIDAD);

const estaRegularidadVencida = (registro) => {
  if (registro?.estado !== 'regularizada') return false;

  const fechaVencimiento = calcularFechaVencimientoRegularidad(registro);
  if (!fechaVencimiento) return false;

  return new Date(fechaVencimiento).getTime() < Date.now();
};

const obtenerEstadoVigenteRegistro = (registro) => {
  if (!registro?.estado) return null;
  return estaRegularidadVencida(registro) ? 'no_cursada' : registro.estado;
};
...
const obtenerTodosLosEstados = async (estudianteId) => {
  ...
  const estadosPorEquivalencia = new Map();

  registros.forEach((r) => {
    const estadoVigente = obtenerEstadoVigenteRegistro(r);

    map.set(r.materiaId, estadoVigente);
    const codigo = normalizarTexto(r.Materia?.codigo);
    if (codigo) {
      const previoCodigo = estadosPorCodigo.get(codigo);
      estadosPorCodigo.set(
        codigo,
        estadoMasFavorable(previoCodigo, estadoVigente)
      );
    }

    const claveEquivalencia = obtenerClaveEquivalenciaMateria({
      codigo: r.Materia?.codigo,
      nombre: r.Materia?.nombre,
    });

    if (claveEquivalencia) {
      const previoEquivalencia = estadosPorEquivalencia.get(claveEquivalencia);
      estadosPorEquivalencia.set(
        claveEquivalencia,
        estadoMasFavorable(previoEquivalencia, estadoVigente)
      );
    }
  });

  map.porCodigo = estadosPorCodigo;
  map.porEquivalencia = estadosPorEquivalencia;
  return map;
};
...
const obtenerEstadoMateria = (estadosMap, materia) => {
  ...
  const estadoPorCodigo =
    codigo && estadosMap.porCodigo ? estadosMap.porCodigo.get(codigo) : null;
  const claveEquivalencia = obtenerClaveEquivalenciaMateria(materia || {});
  const estadoPorEquivalencia =
    claveEquivalencia && estadosMap.porEquivalencia
      ? estadosMap.porEquivalencia.get(claveEquivalencia)
      : null;

  return [estadoDirecto, estadoPorCodigo, estadoPorEquivalencia].reduce(
    (acumulado, estadoActual) => estadoMasFavorable(acumulado, estadoActual),
    null
  );
};
...
const obtenerEstadoConEquivalencias = (estadosMap, materiaRef) => {
  ...
  const estadoPorCodigo = materiaRef?.codigo
    ...
    : null;
  const claveEquivalencia = obtenerClaveEquivalenciaMateria(materiaRef || {});
  const estadoPorEquivalencia = claveEquivalencia
    ? estadosMap.porEquivalencia?.get(claveEquivalencia)
    : null;

  return [estadoPorId, estadoPorCodigo, estadoPorEquivalencia].reduce(
    (acumulado, estadoActual) => estadoMasFavorable(acumulado, estadoActual),
    null
  );
};

const crearMetadataRegularidad = (registroEstado) => {
  const fechaRegularidad = obtenerFechaEstado(registroEstado);
  const fechaVencimientoRegularidad = calcularFechaVencimientoRegularidad(
    registroEstado
  );
  const regularidadVencida = estaRegularidadVencida(registroEstado);

  return {
    fechaRegularidad,
    fechaVencimientoRegularidad,
    regularidadVencida,
  };
};
...
const puedeVerSituacionAcademica = async (
  estudianteId,
  currentEstudianteId
) => {
  if (!currentEstudianteId) return false;

  const esPropioPerfil = Number(estudianteId) === Number(currentEstudianteId);
  if (esPropioPerfil) return true;

  const esContacto = await Conexion.findOne({
    where: {
      [Op.or]: [
        { usuarioId: currentEstudianteId, contactoId: estudianteId },
        { usuarioId: estudianteId, contactoId: currentEstudianteId },
      ],
      estado: 'aceptada',
    },
  });
...
export const getPlanMaterias = async (req, res) => {
  try {
    ...
    estadosTemp.porCodigo = new Map(estadosBD.porCodigo || []);
    estadosTemp.porEquivalencia = new Map(estadosBD.porEquivalencia || []);
      ...
      materiasPorAnioTemp[anio].forEach((materia) => {
        ...
        if (estadoGuardado) {
          if (
            ...
            const codigoMateria = normalizarTexto(materia.codigo);
            if (codigoMateria) {
              estadosTemp.porCodigo.set(codigoMateria, 'no_cursada');
            }
            const claveEquivalenciaMateria = obtenerClaveEquivalenciaMateria(
              materia
            );
            if (claveEquivalenciaMateria) {
              estadosTemp.porEquivalencia.set(
                claveEquivalenciaMateria,
                'no_cursada'
              );
            }
          } else {
            ...
            const codigoMateria = normalizarTexto(materia.codigo);
            if (codigoMateria) {
              estadosTemp.porCodigo.set(codigoMateria, estadoGuardado);
            }
            const claveEquivalenciaMateria = obtenerClaveEquivalenciaMateria(
              materia
            );
            if (claveEquivalenciaMateria) {
              const estadoPrevioEquivalente = estadosTemp.porEquivalencia.get(
                claveEquivalenciaMateria
              );
              estadosTemp.porEquivalencia.set(
                claveEquivalenciaMateria,
                estadoMasFavorable(estadoPrevioEquivalente, estadoGuardado)
              );
            }
          }
        } else {
          ...
          const codigoMateria = normalizarTexto(materia.codigo);
          if (codigoMateria) {
            const estadoPrevioCodigo = estadosTemp.porCodigo.get(codigoMateria);
            estadosTemp.porCodigo.set(
              codigoMateria,
              estadoMasFavorable(estadoPrevioCodigo, 'no_cursada')
            );
          }
          const claveEquivalenciaMateria = obtenerClaveEquivalenciaMateria(
            materia
          ...
          if (claveEquivalenciaMateria) {
            ...
            estadosTemp.porEquivalencia.set(
              claveEquivalenciaMateria,
              estadoMasFavorable(estadoPrevioEquivalente, 'no_cursada')
            );
          }
        }
      });
    ...
    const materiasConEstado = materias.map((materia) => {
      ...
      const registroEstado = registros.find((r) => r.materiaId === materia.id);
      ...
      return {
        ...
        ...crearMetadataRegularidad(registroEstado),
      };
    });
    ...
    if (['cursando', 'regularizada', 'aprobada'].includes(estado)) {
      ...
      try {
        ...
        if (!preferencias || preferencias[prefKey] !== false) {
          novedadCreada = await Novedad.create({
            tipo,
            titulo: `${
              tipo === 'inscripcion'
                ? 'Se inscribió a'
      ...
      if (estado === 'aprobada') {
        try {
          ...
          const idsContactos = conexionesEst
            .map((c) =>
              Number(c.usuarioId) === Number(estudiante.id)
                ? c.contactoId
                : c.usuarioId
            )
            .filter((id) => Number(id) !== Number(estudiante.id));
...
export const getMateriasIds = async (req, res) => {
  try {
    ...
    const scope = await getEstudianteMateriaScope({ usuarioId });

    if (!scope.estudianteId) {
      return res.status(404).json({
        error: 'Estudiante no encontrado',
      });
    }

    res.json({ data: scope.materiaIds });
  } catch (error) {
    res.status(500).json({
      message: 'Error al obtener las materias del estudiante',
      error: error.message,
...
export const importarMateriasDesdeExcel = async (req, res) => {
  try {
    ...
    const carrerasEstudiante = estudiante.Carreras || [];
    ...
    let carrerasObjetivo = carrerasEstudiante;
    if (carreraId) {
      carrerasObjetivo = carrerasEstudiante.filter(
        (c) => Number(c.id) === Number(carreraId)
      );
      if (carrerasObjetivo.length === 0) {
        return res.status(400).json({
    ...
    const planesObjetivo = [];

    if (planId) {
      const planEncontrado = carrerasObjetivo
        .flatMap((c) => c.PlanDeEstudios || [])
        .find((p) => Number(p.id) === Number(planId));

      if (!planEncontrado) {
        return res.status(400).json({
          message:
            'El plan de estudio indicado no pertenece a una carrera del estudiante.',
        });
      }

      planesObjetivo.push(planEncontrado);
    } else {
      carrerasObjetivo.forEach((carreraItem) => {
        const planesCarrera = carreraItem.PlanDeEstudios || [];
        const planVigente =
          planesCarrera.find((p) => p.estado === 'vigente') ||
          planesCarrera[0] ||
          null;

        if (planVigente) {
          planesObjetivo.push(planVigente);
        }
      });
    }

    if (planesObjetivo.length === 0) {
      return res.status(400).json({
        message:
          'No se encontró un plan de estudio vigente para las carreras seleccionadas del estudiante.',
      });
    }

    const planIdsObjetivo = [...new Set(planesObjetivo.map((p) => p.id))];
      ...
      include: [
        {
          model: PlanDeEstudio,
          through: { attributes: ['anio'] },
          where: { id: { [Op.in]: planIdsObjetivo } },
        },
      ],
    });
    ...
    for (const fila of filasOrdenadas) {
      ...
      const estadoActual =
        obtenerEstadoConEquivalencias(estadosSimulados, {
          id: materiaId,
          codigo: materiaPlan.codigo,
        }) || 'no_cursada';
      ...
      const incumplidas = prerequisitos.filter((preId) => {
        const materiaPrereq = materiaPorId.get(preId);
        const estadoPrereq =
          obtenerEstadoConEquivalencias(estadosSimulados, {
            id: preId,
            codigo: materiaPrereq?.codigo,
          }) || 'no_cursada';
        return estadoPrereq !== 'aprobada' && estadoPrereq !== 'regularizada';
      });
...
export const getAsistenteAcademico = async (req, res) => {
  try {
    ...
    // Cargar correlatividades dentro del alcance seleccionado.
    const correlatividades = await Correlatividades.findAll({
      where: {
        materiaId: { [Op.in]: materiasIds },
    ...
    const registrosEstadosPorEquivalencia = new Map();
    registrosEstados.forEach((registro) => {
      ...
      const claveEquivalencia = obtenerClaveEquivalenciaMateria({
        codigo: registro.Materia?.codigo,
        nombre: registro.Materia?.nombre,
      });
      if (
        claveEquivalencia &&
        !registrosEstadosPorEquivalencia.has(claveEquivalencia)
      ) {
        registrosEstadosPorEquivalencia.set(claveEquivalencia, registro);
      }
    });
    ...
    const getRegistroEstado = (materiaRef) => {
      ...
      if (!claveEquivalencia) return null;
      return registrosEstadosPorEquivalencia.get(claveEquivalencia) || null;
    };

    const estaCumplido = (estado) =>
      estado === 'aprobada' || estado === 'regularizada';

    const agruparPrerequisitosPorCarrera = (materiaRef, prerequisitos) => {
      const carrerasMateria = materiaRef?.carreras || [];
      if (!Array.isArray(carrerasMateria) || carrerasMateria.length <= 1) {
        return [];
      }

      const grupos = carrerasMateria
        .map((carreraNombre) => {
          const prerequisitosCarrera = prerequisitos.filter((preId) => {
    ...
    const prerequisitosCumplidosConEstado = (materiaRef, obtenerEstadoFn) => {
      const prerequisitos = correlatividadesMap[materiaRef.id] || [];
      ...
      if (gruposPorCarrera.length > 1) {
        return gruposPorCarrera.some((grupo) =>
          grupo.every((preId) => estaCumplido(obtenerEstadoFn(preId)))
        );
      }

      return prerequisitos.every((preId) =>
        estaCumplido(obtenerEstadoFn(preId))
      );
    };

    const prerequisitosCumplidos = (materiaId) => {
      const materiaRef = materiasPorId.get(materiaId);
      if (!materiaRef) return false;
      return prerequisitosCumplidosConEstado(materiaRef, getEstado);
    };
    ...
    // Finales pendientes: consolidar materias equivalentes para no duplicar avisos.
    const finalesPendientesMap = new Map();
    regularizadas.forEach((m) => {
      const claveEquivalencia =
        obtenerClaveEquivalenciaMateria({
          codigo: m.codigo,
          nombre: m.nombre,
        }) || `id:${m.id}`;

      const registroEstado = getRegistroEstado(m);
      const fechaRegularidad =
        registroEstado?.updatedAt || registroEstado?.createdAt || null;
      const fechaVencimientoRegularidad = sumarAnios(fechaRegularidad, 2);

      const existente = finalesPendientesMap.get(claveEquivalencia);
      if (!existente) {
        finalesPendientesMap.set(claveEquivalencia, {
          id: m.id,
          nombre: m.nombre,
          anio: m.anio,
          cargaHoraria: m.cargaHoraria || 0,
      ...
      finalesPendientesMap.set(claveEquivalencia, {
        ...existente,
        carreras: [
          ...new Set([...(existente.carreras || []), ...(m.carreras || [])]),
        ],
      });
    });

    const finalesPendientes = Array.from(finalesPendientesMap.values());
    ...
    estadosHipoteticos.porCodigo = new Map(estadosBD.porCodigo || []);
    estadosHipoteticos.porEquivalencia = new Map(
      estadosBD.porEquivalencia || []
    );
    cursando.forEach((m) => {
      estadosHipoteticos.set(m.id, 'regularizada');
      const codigo = normalizarTexto(m.codigo);
      if (codigo) {
        estadosHipoteticos.porCodigo.set(codigo, 'regularizada');
      }
      const claveEquivalencia = obtenerClaveEquivalenciaMateria(m);
      if (claveEquivalencia) {
        estadosHipoteticos.porEquivalencia.set(
          claveEquivalencia,
          'regularizada'
        );
      }
    ...
    const getEstadoHipotetico = (id) => {
      const materiaRef = materiasPorId.get(id);
      return (
        obtenerEstadoConEquivalencias(estadosHipoteticos, {
          id,
          codigo: materiaRef?.codigo,
          nombre: materiaRef?.nombre,
        }) || 'no_cursada'
    ...
    const prerequisitosCumplidosHipotetico = (materiaId) => {
      const materiaRef = materiasPorId.get(materiaId);
      if (!materiaRef) return false;
      return prerequisitosCumplidosConEstado(materiaRef, getEstadoHipotetico);
    };
...
export const update = async (req, res) => {
  try {
    ...
    const { nombre, apellido, email, fechaNacimiento, genero } = req.body;
...
export const uploadAvatar = async (req, res) => {
  try {
    ...
    if (usuario.avatarUrl) {
      try {
        const publicId = usuario.avatarUrl.split('/').pop().split('.')[0];
        await cloudinary.uploader.destroy(`avatars/${publicId}`);
      } catch (err) {
        console.error('Error al eliminar avatar anterior de Cloudinary:', err);
      }
    }

    const avatarUrl = req.file.path;
    await usuario.update({ avatarUrl });

    res.json({
      message: 'Avatar actualizado exitosamente',
      data: { id: estudiante.id, avatarUrl: usuario.avatarUrl },
    });
  } catch (error) {
    res.status(500).json({
