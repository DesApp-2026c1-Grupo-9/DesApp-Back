  ...
  try {
    ...
    if (
      ...
      const prerequisitosCumplidos = (materiaIdToCheck) => {
        ...
        return prerreqs.every((pre) => {
          const estadoPrereq =
            estadosBDUpdate.get(pre.id) ||
            (pre.codigo
              ? estadosBDUpdate.porCodigo?.get(normalizarTexto(pre.codigo))
              : null) ||
            'no_cursada';
...
export const importarMateriasDesdeExcel = async (req, res) => {
  try {
    ...
    const nombreAId = new Map(
      materiasPlan.map((m) => [normalizarTexto(m.nombre), m.id])
    );
    ...
    const filasOrdenadas = materias
      .map((fila, indice) => {
        const materiaPlan = materiaPorNombre.get(normalizarTexto(fila.nombre));
    ...
    for (const fila of filasOrdenadas) {
      ...
      if (!materiaId) {
        resultados.ignoradas.push({
          fila,
          razon: `Materia no encontrada en el plan vigente: "${fila.nombre}"`,
        });
        continue;
      }
      ...
      if (incumplidas.length > 0) {
        resultados.ignoradas.push({
          fila,
          razon: 'No cumple correlatividades del plan vigente',
        });
        continue;
      }
