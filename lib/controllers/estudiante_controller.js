  ...
  try {
    ...
    const filasOrdenadas = materias
      .map((fila, indice) => {
        ...
        if (!materiaPlan && fila.nombre) {
          materiaPlan = materiaPorNombre.get(normalizarTexto(fila.nombre)) || null;
        }
    ...
    for (const fila of filasOrdenadas) {
      ...
      if (incumplidas.length > 0) {
        // Si la materia ya tenía un estado (cursando/regularizada), permitir
        // solo si no está en no_cursada (ya fue aprobada en el pasado)
