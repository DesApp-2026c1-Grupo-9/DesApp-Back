...
export const index = async (req, res) => {
  ...
  const comentariosConLikes = comentarios.rows.map((c) => {
    ...
    const liked = userId
      ...
      : false;
    const likesDetails = comentarioLikes
      .map((l) => l.usuario)
      .filter((u) => u !== null);
    ...
    if (data.respuestas) {
      data.respuestas = data.respuestas.map((r) => {
        ...
        return {
          ...
          liked: rLiked,
          likes: rLikes.map((l) => Number(l.usuarioId)),
          likesDetails: rLikesDetails,
        };
      });
    }
    ...
    return {
      ...
      likesCount,
      liked,
      likes: comentarioLikes.map((l) => Number(l.usuarioId)),
      likesDetails,
    };
  });
