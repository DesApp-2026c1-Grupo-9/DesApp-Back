...
export const getPendientes = async (req, res) => {
  ...
  const pendientesTransformadas = pendientes.map((p) => {
    ...
    if (data.contacto) {
      data.contacto = {
        ...data.contacto.Usuario,
        estudianteId: data.contacto.id,
      };
    }
    return data;
  });
...
export const invite = async (req, res) => {
  ...
  const contactoEst = await Estudiante.findOne({
    where: { usuarioId: contacto.id },
  });
  if (!contactoEst) {
    return res.status(400).json({
...
export const remove = async (req, res) => {
  ...
  if (
    conexion.usuarioId !== estudianteId &&
    conexion.contactoId !== estudianteId
  ) {
    return res.status(403).json({
