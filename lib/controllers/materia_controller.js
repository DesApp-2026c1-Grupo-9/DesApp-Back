...
const normalizarNombreMateria = (valor) =>
  String(valor || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
const buscarMateriaDuplicadaPorNombre = async (nombre, excluirId = null) => {
  const nombreNormalizado = normalizarNombreMateria(nombre);
  ...
  return (
    materias.find((m) => {
      if (excluirId && Number(m.id) === Number(excluirId)) return false;
      return normalizarNombreMateria(m.nombre) === nombreNormalizado;
    }) || null
  );
};
...
export const index = async (req, res) => {
  try {
    ...
    if (estudianteId || usuarioId) {
      const scope = await getEstudianteMateriaScope({
        estudianteId,
        usuarioId,
      });
...
export const create = async (req, res) => {
  try {
    ...
    const materiaDuplicada = await buscarMateriaDuplicadaPorNombre(nombre);
    if (materiaDuplicada) {
      return res.status(409).json({
        message: `Ya existe una materia equivalente: "${materiaDuplicada.nombre}". Para reutilizarla, asignala desde la pestaña de carreras/planes.`,
        materiaExistente: {
          id: materiaDuplicada.id,
          nombre: materiaDuplicada.nombre,
        },
      });
    }
...
export const update = async (req, res) => {
  try {
    ...
    if (
      nombre &&
      normalizarNombreMateria(nombre) !==
        normalizarNombreMateria(materia.nombre)
    ) {
      const materiaDuplicada = await buscarMateriaDuplicadaPorNombre(
        nombre,
        id
      );
      if (materiaDuplicada) {
        return res.status(409).json({
          message: `No se puede renombrar: ya existe una materia equivalente "${materiaDuplicada.nombre}".`,
          materiaExistente: {
            id: materiaDuplicada.id,
            nombre: materiaDuplicada.nombre,
          },
