...
export const index = async (req, res) => {
  const { materiaId, fecha, tipo, incluirCanceladas } = req.query;
  ...
  const sesionWhere = incluirCanceladas === 'true' ? {} : { estado: 'activa' };
...
export const create = async (req, res) => {
  ...
  try {
    const conexiones = await Conexion.findAll({
      where: {
        [Op.or]: [{ usuarioId: creadorId }, { contactoId: creadorId }],
        estado: 'aceptada',
      },
    });
    ...
    const idsConexiones = conexiones
      .map((c) =>
        Number(c.usuarioId) === Number(creadorId) ? c.contactoId : c.usuarioId
      )
      .filter((id) => Number(id) !== Number(creadorId));

    for (const contactoId of idsConexiones) {
      await crearNotificacion({
        usuarioId: contactoId,
        tipo: 'sesion_creada_conexion',
        titulo: `${materia.nombre}: ${tema}`,
        actorId: creadorId,
        materiaId,
        sesionId: sesion.id,
      });
    ...
    const cursando = await EstudianteMateria.findAll({
      ...
      include: [{ model: Estudiante, attributes: ['usuarioId'] }],
    });

    for (const em of cursando) {
      const uid = em.Estudiante?.usuarioId;
      if (uid && Number(uid) !== Number(creadorId)) {
        await crearNotificacion({
          usuarioId: uid,
