...
export const create = async (req, res) => {
  ...
  if (!creadorId) return res.status(400).json({ error: 'Se requiere usuarioId' });

  const {
    materiaId, tema, tipo, link, ubicacion, fechaHora,
    duracion, cupos, descripcion, necesidadAprobacion
  } = req.body;

  if (tipo === 'virtual' && !link) return res.status(400).json({ error: 'Link requerido para sesiones virtuales' });
  if (tipo === 'presencial' && !ubicacion) return res.status(400).json({ error: 'Ubicacion requerida para sesiones presenciales' });

  const materia = await Materia.findByPk(materiaId);
  if (!materia) return res.status(404).json({ error: 'Materia no encontrada' });

  const sesion = await Sesion.create({
    materiaId, creadorId, tema, tipo, link, ubicacion,
    fechaHora, duracion, cupos, descripcion,
...
export const update = async (req, res) => {
  ...
  const { tema, tipo, link, ubicacion, fechaHora, duracion, cupos, descripcion, necesidadAprobacion } = req.body;

  const sesion = await Sesion.findByPk(id);
  if (!sesion) return res.status(404).json({ error: 'Sesion no encontrada' });
  if (sesion.creadorId !== creadorId) return res.status(403).json({ error: 'No autorizado' });
  if (sesion.estado === 'cancelada') return res.status(400).json({ error: 'No se puede editar una sesion cancelada' });

  if (tipo === 'virtual' && !link) return res.status(400).json({ error: 'Link requerido para sesiones virtuales' });
  if (tipo === 'presencial' && !ubicacion) return res.status(400).json({ error: 'Ubicacion requerida para sesiones presenciales' });

  await sesion.update({
    tema, tipo, link, ubicacion, fechaHora, duracion, cupos, descripcion, necesidadAprobacion
  });
...
export const inscribirse = async (req, res) => {
  ...
  if (!estudianteId) return res.status(400).json({ error: 'Se requiere usuarioId' });

  const sesion = await Sesion.findByPk(id, {
    include: [{ model: SesionParticipante, as: 'participantes', where: { estado: 'aprobado' }, required: false }]
  });
  if (!sesion) return res.status(404).json({ error: 'Sesion no encontrada' });
  if (sesion.estado !== 'activa') return res.status(400).json({ error: 'Sesion no activa' });

  const existing = await SesionParticipante.findOne({ where: { sesionId: id, estudianteId } });
  if (existing) return res.status(400).json({ error: 'Ya estas inscrito en esta sesion' });
...
export const getParticipantes = async (req, res) => {
  ...
  const participantes = await SesionParticipante.findAll({
    where: { sesionId: id },
    include: [{ model: Usuario, as: 'estudiante', attributes: ['id', 'nombre', 'apellido', 'email'] }]
  });
...
export const aprobarParticipante = async (req, res) => {
  ...
  const participante = await SesionParticipante.findOne({ where: { id: participanteId, sesionId: id } });
...
export const leaveSesion = async (req, res) => {
  ...
  if (participante.estudianteId !== estudianteId) {
    return res.status(403).json({ error: 'No autorizado para abandonar esta inscripcion' });
  }
