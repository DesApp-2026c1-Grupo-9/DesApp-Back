...
const flattenParticipantes = (json) => {
  if (json.participantes) {
    json.participantes = json.participantes.map((p) => flattenCreador(p));
  }
  return json;
};
...
export const index = async (req, res) => {
  ...
  const paginatedSesiones = visibleSesiones.slice(0, parseInt(limit)).map((s) =>
    flattenParticipantes(flattenCreador(s.toJSON()))
  );
