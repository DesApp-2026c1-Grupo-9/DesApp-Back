...
export const create = async (req, res) => {
  ...
  const createdSesion = await Sesion.findByPk(sesion.id, {
    include: [
      creadorInclude,
      { model: Materia, as: 'Materia', attributes: ['id', 'nombre'] },
      {
  ...
  res
    .status(201)
    .json(flattenParticipantes(flattenCreador(createdSesion.toJSON())));
};
  ...
  try {
    ...
    for (const p of participantes) {
      await crearNotificacion({
        ...
        tipo: 'sesion_cancelada',
        titulo: `${creadorNombre} canceló la sesión "${sesion.tema}"`,
        actorId: estudianteId,
        materiaId: sesion.materiaId,
        sesionId: sesion.id,
    ...
    console.error('Error al notificar cancelación a participantes:', err);
  }
...
export const inscribirse = async (req, res) => {
  ...
  try {
    const [estudianteActor, sesionCompleta] = await Promise.all([
    ...
    const actorNombre = estudianteActor?.Usuario
      ? `${estudianteActor.Usuario.nombre} ${estudianteActor.Usuario.apellido}`
      : `Usuario #${estudianteId}`;

    if (sesionCompleta.creadorId && sesionCompleta.creadorId !== estudianteId) {
      const notifTipo = sesionCompleta.necesidadAprobacion
        ? 'sesion_solicitud_nueva'
        : 'sesion_participante_nuevo';
      const notifTitulo = sesionCompleta.necesidadAprobacion
        ? `${actorNombre} quiere unirse a tu sesión "${sesionCompleta.tema}"`
        : `${actorNombre} se unió a tu sesión "${sesionCompleta.tema}"`;

      await crearNotificacion({
        usuarioId: sesionCompleta.creadorId,
        tipo: notifTipo,
        titulo: notifTitulo,
        actorId: estudianteId,
        materiaId: sesionCompleta.materiaId,
        sesionId: sesionCompleta.id,
        sesionTema: sesionCompleta.tema,
      });
    }
  } catch (err) {
    console.error('Error al notificar al creador de la sesión:', err);
  }
