...
const SELECT_MATERIA_ATTRIBUTES = ['id', 'nombre', 'codigo'];
...
const obtenerMateriasEquivalentes = async (materia) => {
  const claveEquivalencia = obtenerClaveEquivalenciaMateria(materia || {});
  if (!claveEquivalencia && !materia?.id) {
    return [];
  }
  ...
  if (normalizarTexto(materia?.codigo)) {
    return Materia.findAll({
      ...
      attributes: SELECT_MATERIA_ATTRIBUTES,
    });
  }
  ...
  if (claveEquivalencia) {
    ...
    return materiasCandidatas.filter(
      (candidata) =>
        obtenerClaveEquivalenciaMateria(candidata) === claveEquivalencia
    );
  }

  return materia?.id ? [materia] : [];
};

const expandirMateriaIdsEquivalentes = async (materiaIds = []) => {
  const ids = uniqueNumeric(materiaIds);
  if (!ids.length) return [];

  const materiasBase = await Materia.findAll({
    where: { id: { [Op.in]: ids } },
    attributes: SELECT_MATERIA_ATTRIBUTES,
  });

  if (!materiasBase.length) return ids;

  const materiasEquivalentes = await Promise.all(
    materiasBase.map((materia) => obtenerMateriasEquivalentes(materia))
  );

  return uniqueNumeric(
    materiasEquivalentes.flat().map((materia) => materia.id)
  );
};

const deduplicarMateriasPorEquivalencia = (materias = []) => {
  const vistas = new Set();

  return materias.filter((materia) => {
    const clave = obtenerClaveEquivalenciaMateria(materia || {});
    const vista = clave || `id:${materia.id}`;

    if (vistas.has(vista)) return false;
    vistas.add(vista);
    return true;
  });
};
...
export const index = async (req, res) => {
  try {
    ...
    const scope = currentUserId
      ...
      : null;
    const scopeMateriaIds = scope?.materiaIds?.length
      ? await expandirMateriaIdsEquivalentes(scope.materiaIds)
      : [];

    if (!isAdmin && currentUserId) {
      if (!scopeMateriaIds.length) {
        return res.json({ data: [] });
      }

      if (materiaId) {
        const parsedMateriaId = parseInt(materiaId);
        const materiaIdsSeleccionadas = await expandirMateriaIdsEquivalentes([
          parsedMateriaId,
        ]);
        const materiaIdsFiltradas = materiaIdsSeleccionadas.filter((id) =>
          scopeMateriaIds.includes(id)
        );

        if (!materiaIdsFiltradas.length) {
          return res.json({ data: [] });
        }
        where.materiaId = { [Op.in]: materiaIdsFiltradas };
      } else {
        where.materiaId = { [Op.in]: scopeMateriaIds };
      }
    } else if (materiaId) {
      const parsedMateriaId = parseInt(materiaId);
      const materiaIdsSeleccionadas = await expandirMateriaIdsEquivalentes([
      ...
      where.materiaId = {
        [Op.in]: materiaIdsSeleccionadas.length
          ? materiaIdsSeleccionadas
          : [parsedMateriaId],
      };
    }
...
export const getMaterias = async (req, res) => {
  try {
    ...
    if (estudianteId) {
      ...
      where.id = { [Op.in]: scopeMateriaIds };
    }
    ...
    res.json({ data: deduplicarMateriasPorEquivalencia(materias) });
  } catch (error) {
    res
      .status(500)
...
export const create = async (req, res) => {
  try {
    ...
    const scopeMateriaIds = await expandirMateriaIdsEquivalentes(
      scope.materiaIds
    );
    const materiaIdsEquivalentes = await expandirMateriaIdsEquivalentes([
      Number(materiaId),
    ]);

    const puedePublicar = materiaIdsEquivalentes.some((id) =>
      scopeMateriaIds.includes(id)
    );
