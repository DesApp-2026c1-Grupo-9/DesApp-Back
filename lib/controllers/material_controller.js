...
export const index = async (req, res) => {
  try {
    ...
    const result = filteredMateriales.map((m) => {
      ...
      const upvotes = ratings.filter((r) => r.valor === 1).length;
      const downvotes = ratings.filter((r) => r.valor === -1).length;
      const userRating = usuarioId
        ? ratings.find((r) => r.usuarioId === parseInt(usuarioId))
        : null;
...
export const remove = async (req, res) => {
  try {
    ...
    if (material.tipo === 'file' && material.url) {
      try {
        const publicId = material.url.split('/').pop().split('.')[0];
        await cloudinary.uploader.destroy(`materiales/${publicId}`);
      } catch (fileError) {
        console.error(
