  ...
  try {
    ...
    if (sortBy === 'rating_desc' || sortBy === 'rating_asc') {
      filteredMateriales = filteredMateriales.map(m => {
        ...
        let ratio;
        if (totalRatings === 0) {
          // Unrated materials go to bottom
          ratio = sortBy === 'rating_desc' ? -1 : 2;
        } else {
          ratio = upvotes / totalRatings;
        }
      ...
      filteredMateriales.sort((a, b) => {
        if (sortBy === 'rating_desc') {
          // Sort by ratio descending, then by total ratings descending
          const ratioDiff = b._ratio - a._ratio;
          if (ratioDiff !== 0) return ratioDiff;
          return b._totalRatings - a._totalRatings;
        } else {
          // Sort by ratio ascending (worst first), then by total ratings descending
          const ratioDiff = a._ratio - b._ratio;
          if (ratioDiff !== 0) return ratioDiff;
          return b._totalRatings - a._totalRatings;
        }
      });
